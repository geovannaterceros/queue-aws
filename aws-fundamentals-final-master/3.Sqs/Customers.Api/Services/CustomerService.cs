...
public class CustomerService : ICustomerService
{
    ...
    private readonly ISqsMessager _sqsMesseger;

    public CustomerService(ICustomerRepository customerRepository, 
        IGitHubService gitHubService, ISqsMessager sqsMesseger)
    {
        _customerRepository = customerRepository;
        _gitHubService = gitHubService;
        _sqsMesseger = sqsMesseger;
    }
    ...
    public async Task<bool> CreateAsync(Customer customer)
    {
        ...
        if (response)
        {
            await _sqsMesseger.SendMessageAsync(customer.ToCustomerCreatedMessage());
        }
    ...
    public async Task<bool> UpdateAsync(Customer customer)
    {
        ...
        if (response)
        {
            await _sqsMesseger.SendMessageAsync(customer.ToCustomerUpdateMessage());
        }
    ...
    public async Task<bool> DeleteAsync(Guid id)
    {
        ...
        if (response)
        {
            await _sqsMesseger.SendMessageAsync(new CustomerDeleted()
            {
                Id = id
            });
        }
