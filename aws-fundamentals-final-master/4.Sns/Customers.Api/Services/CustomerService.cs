...
public class CustomerService : ICustomerService
{
    ...
    private readonly ISnsMessager _snsMesseger;

    public CustomerService(ICustomerRepository customerRepository, 
        IGitHubService gitHubService, ISnsMessager snsMesseger)
    {
        _customerRepository = customerRepository;
        _gitHubService = gitHubService;
        _snsMesseger = snsMesseger;
    }
    ...
    public async Task<bool> CreateAsync(Customer customer)
    {
        ...
        if (response)
        {
            await _snsMesseger.PublishMessageAsync(customer.ToCustomerCreatedMessage());
        }
    ...
    public async Task<bool> UpdateAsync(Customer customer)
    {
        ...
        if (response)
        {
            await _snsMesseger.PublishMessageAsync(customer.ToCustomerUpdateMessage());
        }
    ...
    public async Task<bool> DeleteAsync(Guid id)
    {
        ...
        if (response)
        {
            await _snsMesseger.PublishMessageAsync(new CustomerDeleted()
            {
                Id = id
            });
        }
